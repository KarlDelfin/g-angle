...
function get_short_title( $atts ) {
  ...
  $short_title= get_post_meta( get_the_ID(), 'Short Title',true );
		$non_home= get_post_meta( get_the_ID(), 'Non Home Title',true );
  ...
  if ($non_home == '') {
		  ...
		  echo $block;
			...
			$block = '<h1 class=h1_nonhome>'.$non_home.'</h1>';
			echo $block;
		}
	}
...
function get_excerpt( $count ) {
    global $post; // Add this line to access the current post
    $permalink = get_permalink($post->ID);
    $excerpt = get_the_content(); // This already uses the current post
    $excerpt = strip_tags($excerpt);
    $excerpt = substr($excerpt, 0, $count);
    $excerpt = substr($excerpt, 0, strripos($excerpt, " "));
    $excerpt = '<p>'.$excerpt.'... </p>'; // Added closing </p>
    return $excerpt;
}
